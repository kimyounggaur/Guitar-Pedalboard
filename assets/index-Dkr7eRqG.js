...
const STORAGE_KEY$1 = "guitar-pedalboard:pedals";
const initialPedals = [
  {
    id: "noise-gate",
    type: "noiseGate",
...
function clonePedals(pedals) {
  return pedals.map(clonePedal);
}
function clonePedal(pedal) {
...
function toStoredPedals(pedals) {
...
function savePedals(pedals) {
  if (!canUseStorage$1()) return;
  window.localStorage.setItem(STORAGE_KEY$1, JSON.stringify(toStoredPedals(pedals)));
}
function readStoredPedals() {
  ...
  try {
    ...
    return parsed.filter(
      (pedal) => !!pedal && typeof pedal.id === "string" && typeof pedal.enabled === "boolean" && typeof pedal.bypassed === "boolean" && typeof pedal.params === "object" && pedal.params !== null
    );
  } catch {
    return null;
  }
}
function mergeStoredPedals(storedPedals) {
...
function normalizeStoredParams(type, defaultParams, storedPedal) {
  ...
  if (type === "noiseGate") {
    params.thresholdDb = typeof storedParams.thresholdDb === "number" ? storedParams.thresholdDb : storedParams.threshold ?? params.thresholdDb;
    params.releaseMs = typeof storedParams.releaseMs === "number" ? storedParams.releaseMs : typeof storedParams.release === "number" ? Math.round(storedParams.release * 1e3) : params.releaseMs;
  }
  if (type === "drive") {
  ...
  if (type === "eq") {
    params.bassGain = typeof storedParams.bassGain === "number" ? storedParams.bassGain : storedParams.low ?? params.bassGain;
    params.midGain = typeof storedParams.midGain === "number" ? storedParams.midGain : storedParams.mid ?? params.midGain;
    params.trebleGain = typeof storedParams.trebleGain === "number" ? storedParams.trebleGain : storedParams.high ?? params.trebleGain;
  }
  if (type === "delay") {
    params.timeMs = typeof storedParams.timeMs === "number" ? storedParams.timeMs : typeof storedParams.time === "number" ? Math.round(storedParams.time * 1e3) : params.timeMs;
    params.tone = normalizePercent(storedParams.tone, Number(params.tone));
  }
  if (type === "reverb") {
    params.highCut = typeof storedParams.highCut === "number" ? storedParams.highCut : typeof storedParams.tone === "number" ? 1e3 + normalizePercent(storedParams.tone, 60) * 110 : params.highCut;
  }
  return params;
}
function movePedal(pedals, oldIndex, newIndex) {
  if (oldIndex === newIndex || oldIndex < 0 || newIndex < 0 || oldIndex >= pedals.length || newIndex >= pedals.length) {
...
const usePedalStore = create((set, get) => ({
  pedals: mergeStoredPedals(readStoredPedals()),
  activePedalId: null,
  draggingPedalId: null,
  draggingId: null,
  reorderPedals: (oldIndex, newIndex) => set((state) => {
    const pedals = movePedal(state.pedals, oldIndex, newIndex);
    savePedals(pedals);
    return { pedals };
  }),
  togglePedal: (id) => set((state) => {
    const pedals = state.pedals.map(
      (pedal) => pedal.id === id ? { ...clonePedal(pedal), enabled: !pedal.enabled } : clonePedal(pedal)
    );
    savePedals(pedals);
    return { pedals };
  }),
  setPedalBypass: (id, bypassed) => set((state) => {
    const pedals = state.pedals.map(
      (pedal) => pedal.id === id ? clonePedal({
        ...pedal,
        bypassed,
        params: {
          ...pedal.params,
  ...
  updatePedalParam: (id, paramName, value) => set((state) => {
    const pedals = state.pedals.map((pedal) => {
      if (pedal.id !== id) return clonePedal(pedal);
      const bypassed = (paramName === "bypass" || paramName === "bypassed") && typeof value === "boolean" ? value : pedal.bypassed;
      const nextValue = pedal.type === "delay" && paramName === "feedback" && typeof value === "number" ? Math.min(0.95, Math.max(0, value)) : value;
      return clonePedal({
        ...pedal,
        bypassed,
        params: {
  ...
  updatePedal: (pedal) => set((state) => {
    const nextPedal = clonePedal(pedal);
    const pedals = state.pedals.map(
      (current) => current.id === pedal.id ? nextPedal : clonePedal(current)
    );
    savePedals(pedals);
    return { pedals };
  }),
...
function ChainText() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "side-panel chain-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel-title", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "eyebrow", children: "Signal Chain" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "체인 순서" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("ol", { className: "chain-list", children: pedals.map((pedal, index) => /* @__PURE__ */ jsxRuntimeExports.jsxs("li", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: index + 1 }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: pedal.name }),
      pedal.bypassed && /* @__PURE__ */ jsxRuntimeExports.jsx("em", { children: "Bypass" })
    ] }, pedal.id)) })
  ] });
}
...
function smoothParam(param, value, context, timeConstant = 0.012) {
  const now = context.currentTime;
  param.cancelScheduledValues(now);
  param.setTargetAtTime(value, now, timeConstant);
}
function rampParam(param, value, context, duration = 0.02) {
  const now = context.currentTime;
  param.cancelScheduledValues(now);
...
class DelayEffect extends BaseEffect {
  ...
  update(pedal) {
    ...
    smoothParam(this.flutterDepth.gain, params.mode === "tape" ? 6e-3 : 0, this.context);
    smoothParam(this.flutter.frequency, params.mode === "tape" ? 4.2 : 0.1, this.context);
  }
  getDelayTime(params) {
    if (params.mode === "slapback") return clamp(params.timeMs / 1e3, 0.02, 0.14);
    if (!params.sync) return clamp(params.timeMs / 1e3, 0.02, 2);
...
function createDriveCurve(mode, drive, bias, samples = 4096) {
...
class DriveEffect extends BaseEffect {
  shaper;
  tone;
  lastMode = null;
  lastDrive = -1;
  ...
  update(pedal) {
    super.update(pedal);
    const params = pedal.params;
    if (params.mode !== this.lastMode || Math.abs(params.drive - this.lastDrive) > 0.5 || Math.abs(params.bias - this.lastBias) > 0.02) {
...
class NoiseGateEffect extends BaseEffect {
  ...
  constructor(context, pedal) {
    ...
    this.gate.port.onmessage = (event) => {
...
function createImpulseResponse(context, seconds, mode) {
...
class ReverbEffect extends BaseEffect {
  ...
  update(pedal) {
    ...
    smoothParam(this.preDelay.delayTime, params.preDelay / 1e3, this.context);
    smoothParam(this.lowCut.frequency, params.lowCut, this.context);
...
class TunerNode {
  ...
  autoCorrelate() {
    ...
    const minOffset = Math.floor(this.sampleRate / 1e3);
...
`;
const noiseGateProcessorSource = `
...
class AudioEngine {
  ...
  createEffect(type, id) {
    ...
    switch (type) {
      ...
      default:
        throw new Error(`알 수 없는 이펙터 타입입니다: ${type}`);
    }
  }
  rebuildChain(pedals) {
    this.currentPedals = this.clonePedals(pedals);
    this.currentPedals.forEach((pedal) => this.pedalStates.set(pedal.id, this.clonePedal(pedal)));
    this.rebuildQueue = this.rebuildQueue.catch(() => void 0).then(() => this.rebuildNow(this.currentPedals));
  }
  setPedalParam(pedalId, paramName, value) {
    const pedal = this.pedalStates.get(pedalId);
    if (!pedal) return;
    const nextPedal = {
      ...pedal,
      bypassed: (paramName === "bypass" || paramName === "bypassed") && typeof value === "boolean" ? value : pedal.bypassed,
      params: {
        ...pedal.params,
        [paramName]: value,
        ...(paramName === "bypass" || paramName === "bypassed") && typeof value === "boolean" ? { bypassed: value } : {}
      }
    };
  ...
  setPedalBypass(pedalId, bypassed) {
  ...
  replaceCurrentPedal(pedal) {
    this.currentPedals = this.currentPedals.map(
      (current) => current.id === pedal.id ? this.clonePedal(pedal) : current
    );
  }
}
const silentLevel = {
...
const emptyPitch = { frequency: null, note: null, cents: 0 };
...
async function connectWithDevice(deviceId, pedals) {
  await AudioEngine.getInstance().start(pedals, deviceId || void 0);
...
function DeviceSelector() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "device-selector", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("label", { htmlFor: "audio-device", children: "입력 장치" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "device-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "select",
        {
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "", children: "기본 입력 장치" }),
            devices.map((device, index) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: device.deviceId, children: device.label || `Audio input ${index + 1}` }, device.deviceId))
          ]
        }
      ),
...
function ConnectGuitarPanel() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "side-panel connect-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel-title", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "eyebrow", children: "Audio Input" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "기타 입력 연결" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(DeviceSelector, {}),
    inputMode !== "file" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "connect-actions", children: !isRunning ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        type: "button",
        ...
        children: isLoading ? "연결 중..." : "기타 연결하기"
      }
    ) : /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
    ...
    isRunning && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "panic-button", onClick: panic, children: "Panic" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "audio-file-upload", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { htmlFor: "audio-file", children: "음원 파일" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          id: "audio-file",
      ...
      inputMode === "file" && uploadedFileName && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "file-playback-row", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: uploadedFileName }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            type: "button",
    ...
    isClipping && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "warning-message", children: "입력 신호가 너무 큽니다. 오디오 인터페이스의 input gain을 낮춰 clipping을 피하세요." }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "panel-copy", children: "초기 테스트는 헤드폰 사용을 권장합니다. 오디오 인터페이스의 Direct Monitor가 켜져 있으면 생톤과 처리음이 함께 들릴 수 있습니다." }),
...
function Meter() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "side-panel meter-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel-title", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "eyebrow", children: "Monitor" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "미터 & 튜너" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(LevelBar, { label: "Input", db: inputLevel.db, peakDb: inputLevel.peakDb, value: inputLevel.linear, peak: inputLevel.peakLinear, clipping: inputLevel.isClipping }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(LevelBar, { label: "Output", db: outputLevel.db, peakDb: outputLevel.peakDb, value: outputLevel.linear, peak: outputLevel.peakLinear, clipping: outputLevel.isClipping }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `clip-indicator${isClipping ? " is-active" : ""}`, children: isClipping ? "Clipping" : "Headroom OK" }),
    outputNearClip && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "warning-message", children: "Master output이 0dBFS에 가깝습니다. pedal level 또는 interface output을 낮추세요." }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "cpu-meter", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "control-row", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "meter-track", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { transform: `scaleX(${Math.min(1, cpuLoad / 100)})` } }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tuner-display", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tuner-note", children: pitch.note ?? "--" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: pitch.frequency ? `${pitch.frequency.toFixed(1)} Hz` : "신호 대기" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "cent-strip", "aria-label": "튜닝 센트", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { left: `${Math.min(100, Math.max(0, 50 + pitch.cents))}%` } }) })
    ] })
  ] });
}
function LevelBar({ label, db, peakDb, value, peak, clipping }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "level-row", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "control-row", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `meter-track${clipping ? " is-clipping" : ""}`, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { transform: `scaleX(${value})` } }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("i", { style: { left: `${Math.min(100, Math.max(0, peak * 100))}%` } })
    ] })
  ] });
}
...
function ToggleSwitch({ label, checked, onChange }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "toggle-switch", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: label }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "input",
      {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "switch-track", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "switch-thumb" }) })
  ] });
}
function SliderControl({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "slider-control", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "control-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: label }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("output", { children: shownValue })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "input",
      {
...
function NoiseGatePedal({ params, onChange }) {
  const [gateState, setGateState] = reactExports.useState("Closed");
  reactExports.useEffect(() => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `effect-status state-${gateState.toLowerCase()}`, children: [
      "Gate: ",
      gateState
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
...
function CompressorPedal({ params, onChange }) {
  const estimatedReduction = Math.min(24, Math.max(0, params.sustain / 100 * 14 + (params.ratio - 1) * 0.35));
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "gain-reduction-meter", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "control-row", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Gain Reduction" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("output", { children: [
          "-",
          estimatedReduction.toFixed(1),
          " dB"
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "meter-track", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { transform: `scaleX(${estimatedReduction / 24})` } }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      SliderControl,
      {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      SliderControl,
      {
        ...
        max: 0.1,
        step: 1e-3,
        displayValue: `${Math.round(params.attack * 1e3)} ms`,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      SliderControl,
      {
        ...
        max: 1,
        step: 0.01,
        displayValue: `${Math.round(params.release * 1e3)} ms`,
...
function WaveformCanvas({ input, output }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "waveform-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "control-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Waveform" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("output", { children: "In / Out" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("canvas", { ref: canvasRef, width: 240, height: 84, "aria-label": "Drive input and output waveform" })
  ] });
}
function drawWaveform(context, data, width, height, color) {
...
function DrivePedal({ params, onChange }) {
  ...
  const applyPreset = (name) => {
    const preset = presets$1[name];
    Object.entries(preset).forEach(([key, value]) => onChange(key, value));
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(WaveformCanvas, { input: inputWaveform, output: outputWaveform }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "select-control", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Mode" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: params.mode, onChange: (event) => onChange("mode", event.currentTarget.value), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "overdrive", children: "Overdrive" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "crunch", children: "Crunch" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "distortion", children: "Distortion" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "fuzz", children: "Fuzz" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "preset-buttons", children: Object.keys(presets$1).map((name) => /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "text-button", onClick: () => applyPreset(name), children: name }, name)) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      SliderControl,
      {
        label: "Drive",
...
function EQPedal({ params, onChange }) {
  const applyPreset = (name) => {
    Object.entries(presets[name]).forEach(([key, value]) => {
      if (value !== void 0) onChange(key, value);
    });
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(EQCurve, { params }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "preset-buttons", children: Object.keys(presets).map((name) => /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "text-button", onClick: () => applyPreset(name), children: name }, name)) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SliderControl, { label: "Low Cut", value: params.lowCut, min: 40, max: 160, step: 1, unit: " Hz", onChange: (value) => onChange("lowCut", value) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SliderControl, { label: "Bass", value: params.bassGain, min: -12, max: 12, step: 0.5, unit: " dB", onChange: (value) => onChange("bassGain", value) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SliderControl, { label: "Mid Freq", value: params.midFreq, min: 250, max: 1500, step: 10, unit: " Hz", onChange: (value) => onChange("midFreq", value) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SliderControl, { label: "Mid", value: params.midGain, min: -12, max: 12, step: 0.5, unit: " dB", onChange: (value) => onChange("midGain", value) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SliderControl, { label: "Mid Q", value: params.midQ, min: 0.3, max: 4, step: 0.1, onChange: (value) => onChange("midQ", value) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SliderControl, { label: "Treble", value: params.trebleGain, min: -12, max: 12, step: 0.5, unit: " dB", onChange: (value) => onChange("trebleGain", value) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SliderControl, { label: "Presence", value: params.presenceGain, min: -12, max: 12, step: 0.5, unit: " dB", onChange: (value) => onChange("presenceGain", value) })
  ] });
}
function EQCurve({ params }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { className: "eq-curve", viewBox: "0 0 160 68", role: "img", "aria-label": "EQ curve", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M0 34H160" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("polyline", { points: points.join(" ") })
  ] });
}
function DelayPedal({ params, onChange }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "select-control", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Mode" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: params.mode, onChange: (event) => onChange("mode", event.currentTarget.value), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "digital", children: "Digital" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "analog", children: "Analog" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "tape", children: "Tape" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "slapback", children: "Slapback" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "pingpong", children: "Pingpong" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(ToggleSwitch, { label: "Sync", checked: params.sync, onChange: (checked) => onChange("sync", checked) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      SliderControl,
      {
        label: "Time",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(SliderControl, { label: "Tone", value: params.tone, min: 0, max: 100, step: 1, unit: "%", onChange: (value) => onChange("tone", value) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SliderControl, { label: "BPM", value: params.bpm, min: 40, max: 240, step: 1, onChange: (value) => onChange("bpm", value) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "select-control", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Division" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: params.division, onChange: (event) => onChange("division", event.currentTarget.value), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "1/4", children: "1/4" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "1/8", children: "1/8" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "dotted1/8", children: "Dotted 1/8" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "1/16", children: "1/16" })
      ] })
    ] })
  ] });
}
function ReverbPedal({ params, onChange }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "select-control", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Mode" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: params.mode, onChange: (event) => onChange("mode", event.currentTarget.value), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "room", children: "Room" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "hall", children: "Hall" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "plate", children: "Plate" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "spring", children: "Spring" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "ambient", children: "Ambient" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      SliderControl,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(SliderControl, { label: "Pre Delay", value: params.preDelay, min: 0, max: 200, step: 1, unit: " ms", onChange: (value) => onChange("preDelay", value) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SliderControl, { label: "Low Cut", value: params.lowCut, min: 20, max: 500, step: 5, unit: " Hz", onChange: (value) => onChange("lowCut", value) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SliderControl, { label: "High Cut", value: params.highCut, min: 1e3, max: 12e3, step: 100, unit: " Hz", onChange: (value) => onChange("highCut", value) })
  ] });
}
function PedalCard({ pedal, dragHandleProps, isDragging = false }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "article",
    {
      className: `pedal-card${pedal.bypassed ? " is-bypassed" : ""}${!pedal.enabled ? " is-disabled" : ""}${isDragging ? " is-dragging" : ""}`,
      style: { "--pedal-color": pedal.color },
      onClick: () => setActivePedal(pedal.id),
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "pedal-header", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "pedal-kicker", children: pedal.type }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: pedal.name })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              ...
              pedal.type === "noiseGate" && /* @__PURE__ */ jsxRuntimeExports.jsx(
                NoiseGatePedal,
                {
                  params: pedal.params,
              ...
              pedal.type === "compressor" && /* @__PURE__ */ jsxRuntimeExports.jsx(
                CompressorPedal,
                {
                  params: pedal.params,
                  onChange: (key, value) => commitParam(String(key), value)
              ...
              pedal.type === "drive" && /* @__PURE__ */ jsxRuntimeExports.jsx(
                DrivePedal,
                {
                  params: pedal.params,
                  onChange: (key, value) => commitParam(String(key), value)
              ...
              pedal.type === "eq" && /* @__PURE__ */ jsxRuntimeExports.jsx(
                EQPedal,
                {
                  params: pedal.params,
                  onChange: (key, value) => commitParam(String(key), value)
              ...
              pedal.type === "delay" && /* @__PURE__ */ jsxRuntimeExports.jsx(
                DelayPedal,
                {
                  params: pedal.params,
                  onChange: (key, value) => commitParam(String(key), value)
              ...
              pedal.type === "reverb" && /* @__PURE__ */ jsxRuntimeExports.jsx(
                ReverbPedal,
                {
                  params: pedal.params,
                  onChange: (key, value) => commitParam(String(key), value)
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "footer",
          {
            ...
            onKeyDown: stopControlEvent,
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(ToggleSwitch, { label: "On", checked: pedal.enabled, onChange: toggleEnabled }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                ToggleSwitch,
                {
                  label: "Bypass",
...
function SortablePedal({ pedal }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: setNodeRef, style, className: "sortable-pedal", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
    PedalCard,
    {
      pedal,
      isDragging,
...
function PedalBoard() {
  const [showChainToast, setShowChainToast] = reactExports.useState(false);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "board-section", "aria-label": "페달보드", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section-heading", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "eyebrow", children: "Pedalboard" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { children: "Web Guitar Multi-Effects" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "hint", children: "드래그 종료 후 체인을 재연결합니다" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "signal-chain-text", "aria-live": "polite", children: chainText }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      DndContext,
      {
        sensors,
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsx(SortableContext, { items: pedals.map((pedal) => pedal.id), strategy: horizontalListSortingStrategy, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pedal-board", children: pedals.map((pedal) => /* @__PURE__ */ jsxRuntimeExports.jsx(SortablePedal, { pedal }, pedal.id)) }) })
      }
    ),
    showChainToast && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "chain-toast", role: "status", children: "Signal Chain Updated" })
  ] });
}
const STORAGE_KEY = "web-guitar-pedalboard-presets";
const FACTORY_TIMESTAMP = 17672256e5;
...
function withPedalOverrides(pedal, overrides) {
...
function createPreset(id, name, overrides) {
  return {
    id,
    name,
    isFactory: true,
    updatedAt: FACTORY_TIMESTAMP,
    pedals: clonePedals(initialPedals).map(
      (pedal) => overrides[pedal.id] ? withPedalOverrides(pedal, overrides[pedal.id]) : pedal
    )
  };
}
const defaultPresets = [
  ...
  createPreset("factory-ambient-delay", "Ambient Delay", {
    ...
    drive: { bypassed: true, params: { bypassed: true, mix: 0 } },
    eq: { params: { lowCut: 95, bassGain: -1, midFreq: 1e3, midGain: -1, trebleGain: 2, presenceGain: 3 } },
    delay: {
      params: {
        bypassed: false,
        mode: "tape",
...
function isPreset(value) {
  const preset = value;
  return !!preset && typeof preset.id === "string" && typeof preset.name === "string" && typeof preset.updatedAt === "number" && Array.isArray(preset.pedals);
}
...
function parsePresetJson(json) {
  const parsed = JSON.parse(json);
  const candidates = Array.isArray(parsed) ? parsed : parsed.presets;
...
function writeUserPresets(presets2) {
  if (!canUseStorage()) return;
  window.localStorage.setItem(STORAGE_KEY, JSON.stringify(presets2));
}
function createId() {
  if (globalThis.crypto?.randomUUID) return globalThis.crypto.randomUUID();
  return `preset-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}
function toPresetList(userPresets) {
...
const usePresetStore = create((set, get) => ({
  ...
  savePreset: (name, pedals) => {
    ...
    const nextUserPresets = existing ? userPresets.map((preset) => preset.id === existing.id ? nextPreset : preset) : [nextPreset, ...userPresets];
...
function PresetPanel() {
  const [name, setName] = reactExports.useState("Clean Practice");
  ...
  const presets2 = usePresetStore((state) => state.presets);
  ...
  const exportJson = () => {
    ...
    link.href = url;
    link.download = `guitar-pedalboard-presets-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.json`;
    link.click();
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "side-panel preset-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel-title", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "eyebrow", children: "Preset" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "프리셋" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "preset-form", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          value: name,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "secondary-button", onClick: saveCurrentPreset, children: "저장" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "preset-actions", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "secondary-button", onClick: exportJson, children: "JSON Export" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "secondary-button", onClick: () => importInputRef.current?.click(), children: "JSON Import" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          ref: importInputRef,
    ...
    message && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "preset-message", children: message }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "preset-list", children: presets2.map((preset) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "preset-item", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { type: "button", onClick: () => loadPreset(preset.pedals), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: preset.name }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: preset.isFactory ? "Default Preset" : new Date(preset.updatedAt).toLocaleDateString("ko-KR") })
      ] }),
      !preset.isFactory && /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          type: "button",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "text-button", onClick: reset, children: "기본 체인으로 재설정" })
  ] });
}
function AppShell() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: "app-shell", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "workspace", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(PedalBoard, {}),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "sidebar", "aria-label": "오디오와 프리셋 패널", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(ConnectGuitarPanel, {}),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Meter, {}),
      /* @__PURE__ */ jsxRuntimeExports.jsx(ChainText, {}),
      /* @__PURE__ */ jsxRuntimeExports.jsx(PresetPanel, {})
    ] })
  ] }) });
}
function App() {
